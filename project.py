def main():
    ...
    quantity = int(input("ลูกค้าจะสั่งกี่แก้วคะ : "))

    results = calculator(menu_list[choice]['price'] , sweet[choice_sweet]['price'] , topping[choice_topping]['price'] , size[choice_size]['price'] , quantity)
...
def calculator(price_menu , price_sweet , price_topping , price_size , quan):
    if price_sweet == 'FREE':
        price_sweet = 0
    if price_topping == 'FREE':
        price_topping = 0
    result = (price_menu + price_sweet + price_topping + price_size) * quan
    return result
